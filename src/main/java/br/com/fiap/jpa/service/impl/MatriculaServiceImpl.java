...
private static MatriculaServiceImpl instance = null;
	...
	public List<TotalAlunosCursoDTO> contarAlunosPorCurso(){
		List<TotalAlunosCursoDTO> totalAlunos = null;
		
	try {
		totalAlunos = matriculaDAO.contarAlunosPorCurso(getEntityManager());
	} catch (Exception e) {
		e.printStackTrace();
	} finally {
		closeEntityManager();
